      ...
      if((Boolean) attributes.get("loggedIn"))
      {
          try
          {
              ...
              if(newProjectName != null)
              {
                  ...
                  String newID = newID();
                  stmt.executeUpdate("INSERT INTO users(userID , projects AS userProj) VALUES ('" + userInfo.get("user_id") + "' , '{" + newID + "}') ON CONFLICT(userID) DO UPDATE SET userProj[array_length(userProj, 1) + 1] = '" + newID + "'");
